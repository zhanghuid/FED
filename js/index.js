(function () {
    ...
    search.prototype = {
        // 搜索字符串里面是否存在关键字
        isSearchIndexOF: function (oldstr, kw) {
            var istrue = false;
        ...
        createSearchListHTML: function (keyWolds) {
            var eml = this.boxEml,
                self = this,
                arr,
                total = 0;

            getData('title', keyWolds, function (dt) {
                arr = dt.data.data;
                keywolds = keyWolds.toLowerCase();

                for (var i = 0; i < arr.length; i++) {
                    if (!arr[i]) break;
                    if (self.isSearchIndexOF(arr[i].name, keywolds)
                        || self.isSearchIndexOF(arr[i].des, keywolds)
                    ) {
                        var myLi = document.createElement("LI");
        ...
        createTagsHTML: function (keywolds) {
            ...
            for (var i = 0; i < this.tags.length; i++) {
                ...
                if (self.isSearchIndexOF(this.tags[i], keywolds)) {
        ...
        createTagsListHTML: function (keyWolds) {
            ...
            getData('tags', keyWolds, function (dt) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    ...
                    if (arr[i] && arr[i].tags && this.isSearchIndexOF(arr[i].tags, keywolds)) {
                        var myLi = document.createElement("LI");
        ...
        init: function (dt) {
            ...
            self.info.innerHTML = '搜集到<i> ' + dt.data.total + ' </i>个站点 ｜ ';
    ...
    var url = 'http://fed-admin.local/api/urls';
    var s = new search();
    ...
    var mescroll = new MeScroll("mescroll", {
        ...
        up: {
            ...
            toTop: {
                ...
                offset: 680 //列表滚动1000px才显示回到顶部按钮
            }
        }
    });
    ...
    function upCallback (page) {
        s.page = page.num;
        ajax(url + '?page=' + page.num + '&size=' + page.size, function (dt) {
            s.init(dt);
        });
    }

    function getData (type, kw, callback) {
        ajax(url + '?page='+ s.page + '&type=' + type + '&kw='+ kw, callback);
    }
